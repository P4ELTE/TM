...
/*
 * PI2 AQM is described in:
 *	Paper: https://www.bobbriscoe.net/projects/latency/pi2_conext.pdf
 *
 * Version 1: Fast path implementation
 *
 * TODO: PI controller parameter tuning is required.
 *
*/
                 ...
                 inout standard_metadata_t stdmeta)
{
    ...
    apply {
        ...
        if (qlen_pkts>=cLimit) {
                ...
                /* Classify packet */
                if (hdr.ipv4.isValid() && ((hdr.ipv4.ecn & 1)!=0)) { /* Scalable source */
                   bit<32> rand_val;
                ...
                } else { /* Unscalable source - non-IPv4 packets are also handled here */
                   bit<32> rand_val;
                   bit<32> rand_val2;
                   random<bit<32>>(rand_val, 0, cMaxProb);
                   random<bit<32>>(rand_val2, 0, cMaxProb);
                   if ((rand_val < prob/2) && (rand_val2  < prob/2)) {
                      if (hdr.ipv4.isValid() && ((hdr.ipv4.ecn & 2)!=0)) {
                          /* ECN mark */
                          ecn_mark();
                      } else {
                          mark_to_drop();
